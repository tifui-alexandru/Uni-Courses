...
const std::string sorting_alg[] = {"mergesort", "quicksort", "heapsort", "radixsort", "STL sort"};
double sorting_time[5];
...
void test_sort_alg(int idx, int* v, int N, int* sorted_arr) {
	...
	fout << sorting_alg[idx] << " sorteaza in " << std::setw(6) << sorting_time[idx] << " secunde\n";
}
...
int main() {
	...
	while (no_tests--) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			if (i < 4 and sorting_time[i] < minn) {
				minn = sorting_time[i];
				pos_min = i;
			}
		}
		fout << "Cea mai buna sortare manuala este " << sorting_alg[pos_min] << "\n\n";
